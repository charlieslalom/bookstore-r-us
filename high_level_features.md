...
Feature discovery was performed through systematic analysis of the application codebase, simulating multiple "random walks" through the website structure. The approach included:

### 1. Frontend Route Analysis
Examined the React Router configuration in `react-ui/frontend/src/components/App/index.js` to identify all navigable pages and URL patterns:
- Homepage (`/`)
- Category pages (`/Books`, `/Music`, `/Beauty`, `/Electronics`, `/:category`)
- Product detail pages (`/item/:id`)
- Shopping cart (`/cart`)
- Sort views (`/sort/:query`)

### 2. Navigation Component Inspection
Analyzed the NavBar (`NavBar/index.js`) and Footer (`Footer/index.js`) components to discover all user-facing navigation links, revealing 4 primary categories in the header and 18 extended categories in the footer.

### 3. API Endpoint Mapping
Traced REST API controllers in the microservices:
- `ProductCatalogController.java` - Product listing, details, and category filtering
- `ShoppingCartController.java` - Cart operations and checkout
- `CheckoutController.java` - Order processing
- `UserController.java` - Authentication flows

### 4. Component Deep Dive
Read individual React components to understand feature implementation:
- `Products/index.js` - Pagination, sorting, category filtering
- `ShowProduct/index.js` - Product details and "Also Bought" recommendations
- `Cart/index.js` - Cart management and checkout flow
- `Home/index.js` - Homepage bestseller sections
- `Subscribe/index.js` - Newsletter subscription UI

### 5. Data Model Analysis
Examined domain objects to understand data relationships:
- `ProductMetadata.java` - Product attributes including recommendations (`also_bought`, `also_viewed`)
- `Order.java` - Order structure and fields
- `ShoppingCart.java` - Cart persistence model

### 6. Service Logic Review
Analyzed backend service implementations:
- `CheckoutServiceImpl.java` - Transactional checkout with inventory validation
- `ShoppingCartImpl.java` - Cart state management

This methodology ensured comprehensive coverage of both visible UI features and underlying capabilities exposed through the API layer.
